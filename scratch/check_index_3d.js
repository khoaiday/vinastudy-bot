...
function check() {
    const htmlPath = path.join(__dirname, '..', 'daiviet_defense', 'index_3d.html');
    const content = fs.readFileSync(htmlPath, 'utf8');

    // Simple script tag regex extractor (matches any script tag with attributes)
    ...
    while ((match = scriptRegex.exec(content)) !== null) {
        ...
        try {
            ...
            new vm.Script(jsCode, { filename: `index_3d.html_script_${idx}.js` });
            ...
            if (err.stack) {
                const lineMatch = err.stack.match(/index_3d\.html_script_\d+\.js:(\d+)/);
                if (lineMatch) {
